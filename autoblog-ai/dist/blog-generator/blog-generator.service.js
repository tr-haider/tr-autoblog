...
const web_scraper_service_1 = require("../web-scraper/web-scraper.service");
let BlogGeneratorService = BlogGeneratorService_1 = class BlogGeneratorService {
    configService;
    emailService;
    topicResearchService;
    webScraperService;
    logger = new common_1.Logger(BlogGeneratorService_1.name);
    llm;
    constructor(configService, emailService, topicResearchService, webScraperService) {
        this.configService = configService;
        this.emailService = emailService;
        this.topicResearchService = topicResearchService;
        this.webScraperService = webScraperService;
        const provider = this.configService.get('llm.provider') || 'groq';
        const model = this.configService.get('llm.model') || 'llama-3.1-8b-instant';
    ...
    async generateBlogPost(request) {
        ...
        try {
            ...
            const blogPost = await this.parseBlogResponse(response.content, request);
    ...
    async generateBlogPostAsDocx(request) {
...
[
  {
    ...
    async createBlogPrompt(request) {
        const topics = this.configService.get('blog.topics') || [];
        const allKeywords = this.configService.get('blog.seoKeywords') || [];
        ...
        const relevantLinks = (request.selectedLinks && request.selectedLinks.length > 0)
            ...
            : allLinks.filter(link => request.keywords?.some(keyword => link.title.toLowerCase().includes(keyword.toLowerCase()) ||
                link.description?.toLowerCase().includes(keyword.toLowerCase()))).slice(0, 3);
...
Return ONLY a valid JSON object. DO NOT include the word count as a hardcoded number - I will calculate it from the actual content:
{
  ...
  "createdAt": "2024-01-01T00:00:00.000Z"
    ...
    async parseBlogResponse(response, request) {
        try {
            ...
            const selectedResources = (request.selectedLinks && request.selectedLinks.length > 0)
                ...
                : resources.filter(resource => request.keywords?.some(keyword => resource.title.toLowerCase().includes(keyword.toLowerCase()) ||
                    resource.description?.toLowerCase().includes(keyword.toLowerCase()))).slice(0, 3);
...
<p>Need expert guidance for your healthcare software project? <a href="https://technologyrivers.com/contact-us/">Contact our team</a> for a complimentary consultation.</p>`;
            ...
            switch (element.type) {
                ...
                case 'ul':
                    for (const listItem of element.items || []) {
                        ...
                        children.push(new docx_1.Paragraph({
                            children: [new docx_1.TextRun({ text: '• ' }), ...textRuns],
    ...
    parseTextWithFormatting(text) {
        ...
        return textRuns.length > 0 ? textRuns : [new docx_1.TextRun({ text: text })];
    }
    stripHtmlTags(html) {
        return html
            .replace(/<[^>]*>/g, '')
...
<body>
    ...
    async createDocxFromBlog(blogPost) {
        return this.createDocxDocument(blogPost);
    }
    async createHtmlFromBlog(blogPost) {
        try {
...
exports.BlogGeneratorService = BlogGeneratorService = BlogGeneratorService_1 = __decorate([
